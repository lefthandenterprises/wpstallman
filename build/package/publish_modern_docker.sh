...
if git_root=$(git rev-parse --show-toplevel 2>/dev/null); then
  ...
  # Fallback: assume this script lives at build/package/
  SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
  ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
fi

DOCKERFILE="${DOCKERFILE:-$ROOT/Dockerfile.modern}"   # keep Dockerfile at repo root
...
# (optional) sanity on host before running container:
...
# --- Build the tool image (cached) ---
docker build -f "$DOCKERFILE" -t "$IMAGE_NAME" "$ROOT"

# --- Publish inside container ---
docker run --rm \
  -v "$ROOT":/src \
  -w /src \
  "$IMAGE_NAME" \
...
# --- (Optional) print GLIBC floor detected in the binary ---
